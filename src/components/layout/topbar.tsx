...
import { navItems, navLabel } from "@/components/layout/sidebar";
      ...
      <div className="flex min-h-14 items-center justify-between gap-3 px-3 sm:px-4 md:px-6 lg:h-[60px]">
        <div className="flex min-w-0 flex-1 items-center gap-2">
          ...
          <div className="min-w-0">
            <div className="truncate text-sm font-semibold lg:hidden">{dictionary.common.appName}</div>
            <div className="hidden text-xs font-medium uppercase tracking-normal text-muted-foreground sm:block">
              {dictionary.common.operatorConsole}
            </div>
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-1.5 sm:gap-3">
          ...
          <div className="hidden items-center gap-2 rounded-md border bg-card px-2 py-1 text-sm font-medium sm:flex">
            ...
            <span className="hidden md:inline-block">{dictionary.common.adminUser}</span>
          </div>
          <form action={formAction}>
            <Button variant="outline" size="sm" type="submit" disabled={pending} aria-label={pending ? dictionary.common.loggingOut : dictionary.common.logout}>
              <LogOut className="h-4 w-4 sm:mr-2" aria-hidden="true" />
              <span className="hidden sm:inline">{pending ? dictionary.common.loggingOut : dictionary.common.logout}</span>
            </Button>
          </form>
        </div>
      </div>
      <nav className="flex gap-1 overflow-x-auto border-t border-border/60 px-2 py-2 text-sm font-medium lg:hidden" aria-label={dictionary.nav.adminNavigation}>
        {navItems.map((item) => {
